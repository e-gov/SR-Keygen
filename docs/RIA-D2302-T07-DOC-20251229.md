...
|:----:|:----|
|**AVALIK**| Käesolev dokument on avalik ning mõeldud vabaks kasutamiseks ja levitamiseks. Dokument sisaldab tehnilist teavet ja tööprotseduuride kirjeldusi, mis on koostatud informatiivsel ja juhendaval eesmärgil.<br><br>Dokumendi sisu on koostatud heas usus ning tugineb tööde kirjelduses sätestatud nõuetele ja arendusmeeskonna poolt teostatud tehnilistele tegevustele. Dokument ei sisalda delikaatseid isikuandmeid ega riigisaladust.<br><br>Dokumendis kirjeldatud protseduuride rakendamine ning nende nõuetekohane täitmine on dokumenti kasutava asutuse ja selle volitatud töötajate vastutus. RaulWalter ja dokumendi koostamises osalenud isikud ei vastuta otsese ega kaudse kahju eest, mis võib tuleneda käesoleva dokumendi kasutamisest, tõlgendamisest või selle alusel tehtud otsustest.|
...
| 29.12.2025 | 1.2 | Uuendatud dokumendi turvaklassifikatsioon; lisatud täpsustus YubiKey PIN koodide pikkuse kohta peatükis 5.3; lisatud automatiseeritud SHA256 sõrmejälgede kontrollimise mehhanism peatükis 4.6.5; uuendatud punktis 8.1 USB-mälupulga failistruktuur. |
...
4. Kopeeri käesoleva juhendiga kaasasolev `/install` kataloog ja `sha256_manifest20251229.txt` USB-mälupulga juurkataloogi.
...
5. Kontrolli kõigi installatsioonifailide sha256 sõrmejälgi:

		sha256sum -c ../sha256_manifest20251229.txt
		
	Alternatiivselt, sõrmejälgi on võimalik kontrollida ka käsitsi võrreldes neid Lisas 1 tooduga kasutades käsku:
...
4. *Anna rakendusele käivitusõigus (vajalik vaid FAT32 formaadis USB-mälupulga puhul)*:
...
> **Tähelepanu:**  
> Tulenevalt PIV-standardi piirangutest ning kasutatava YubiKey versiooni tehnilistest omadustest ei tohi PIV-rakenduse PIN-koodi kogupikkus ületada **8 (kaheksat) märki**.  
> 
> Sellest tulenevalt on käesolevas juhendis kirjeldatud kaheosalise PIN-koodi lahendus piiratud selliselt, et **PIN_A** ja **PIN_B** saavad olla kumbki maksimaalselt **4 (neli) märki pikad**, moodustades kokku ühe kehtiva PIV-PIN-koodi.
...
17. *Valikuline: Kontrolli, et slot 9c on Yubikey tokenil täidetud:*
